...
function setCors(res) {
    ...
    res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Accept, X-Requested-With, x-admin-email');
}

// ── ADMIN AUTH CHECK ──────────────────────────────────
function verifyAdmin(req) {
    const adminEmail = req.headers['x-admin-email'] || req.body?.adminEmail;
    return ORION_ADMINS.includes(adminEmail);
}
    ...
    try {
        ...
        if (action === 'sync-user' && req.method === 'POST') {
            ...
            let { data: user } = await supabase
                .from('user_subscriptions').select('*').eq('email', email).single();
        ...
        if (action === 'get-config' && req.method === 'GET') {
            const { data, error } = await supabase
                .from('orion_config').select('*').eq('id', 1).single();
            const defaults = {
        ...
        // ════════════════════════════════════════
        // ADMIN ROUTES — email verify hoti hai
        // ════════════════════════════════════════
        ...
        if (req.method === 'POST' && !action) {
            ...
            if (!email || !textInput) return res.status(400).json({ error: 'Email and input are required.' });
